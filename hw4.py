...
class NnValueFunction:
    ...
    def init_tf(self):
        ...
        sy_h1 = lrelu(dense(self.sy_ob, 128, "v_h1", weight_init=normc_initializer(1.0)))
        sy_h2 = lrelu(dense(sy_h1, 128, "v_h2", weight_init=normc_initializer(1.0)))
...
def main_cartpole(n_iter=100, gamma=1.0, min_timesteps_per_batch=1000, stepsize=1e-2, animate=True,
                  logdir=None, vf_type='linear', vf_params={}):
...
@click.option('--clear-logs/--no-clear-logs', default=False)
def main(task, clear_logs):
    global CLEAR_LOGS
    CLEAR_LOGS = clear_logs

    if task == 0:
        general_params = dict(n_iter=100, gamma=1.0, min_timesteps_per_batch=1000, stepsize=1e-2,
                              animate=False)
        params = [
            dict(logdir='/tmp/cartpole/linear', vf_type='linear', vf_params={}, **general_params),
            dict(logdir='/tmp/cartpole/nn', vf_type='nn', vf_params=dict(n_epochs=10, stepsize=1e-3), **general_params),
        ]
