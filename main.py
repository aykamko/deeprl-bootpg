...
def main(logdir, seed, n_iter, gamma, min_timesteps_per_batch, initial_stepsize, desired_kl,
         vf_type, vf_params, animate=False):
    ...
    sy_mean = dense(sy_h2, ac_dim, 'mean_out', weight_init=normc_initializer(0.1))  # Mean control output
    ...
    sy_old_mean = tf.placeholder(shape=[None, ac_dim], name='old_meana', dtype=tf.float32)
    sy_old_logstd = tf.placeholder(shape=[ac_dim], name='old_logstd', dtype=tf.float32)
    ...
    for i in range(n_iter):
        ...
        ac = np.concatenate([path["action"] for path in paths])
        ...
        logz.log_tabular("EVBefore", explained_variance_1d(vpred, vtarg))
        logz.log_tabular("EVAfter", explained_variance_1d(vf.predict(ob), vtarg))
