...
__all__ = ["fig_to_html", "fig_to_dict", "fig_to_d3",
           ...
           "save_html", "save_json", "DynamicHTML"]
...
def show(fig=None, ip='127.0.0.1', port=8888, n_retries=50,
         local=True, open_browser=True, http_server=None, dynamic=None,
         refresh_rate=-1, **kwargs):
    ...
    **kwargs :
    ...
    if fig is None:
        ...
        fig = plt.gcf()
    if dynamic is None:
        dynamic = DynamicHTML(fig_to_html(fig, **kwargs))
    serve(dynamic, ip=ip, port=port, n_retries=n_retries, files=files,
